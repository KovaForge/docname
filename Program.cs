...
var listNamesScanOption = new Option<string?>("--scan")
{
    Description = "Recursively scan a directory for controlled document filenames and include them in the output."
};
var listNamesCommand = new Command("list-names", "Lists document filenames generated by docname.")
{
    listNamesScanOption
};
listNamesCommand.SetAction(parseResult => ListGeneratedNames(dbPath, parseResult.GetValue(listNamesScanOption)));
...
allocCommand.SetAction(parseResult =>
{
    try
    {
        ...
        var allocatedNumber = GetNextNumber(connection, transaction, l2, l3, allocate: true);
        InsertAllocation(connection, transaction, l2, l3, allocatedNumber, freeText, filename);
        transaction.Commit();
...
root.Subcommands.Add(listNamesCommand);
...
CREATE TABLE IF NOT EXISTS allocations (
    ...
    UNIQUE(level1, level2, level3, number)
);
";
...
int ListGeneratedNames(string dbPath, string? scanDirectory)
{
    try
    {
        InitializeDatabase(dbPath);
        ...
        using (var command = connection.CreateCommand())
        {
            command.CommandText = "SELECT filename FROM allocations ORDER BY level1, level2, level3, number, filename";
            using var reader = command.ExecuteReader();
            while (reader.Read())
            {
        ...
        {
            var expandedScanDirectory = ExpandHome(scanDirectory);
            if (!Directory.Exists(expandedScanDirectory))
            {
                throw new DirectoryNotFoundException($"Scan directory does not exist: {expandedScanDirectory}");
            }

            foreach (var file in Directory.EnumerateFiles(expandedScanDirectory, "*.md", SearchOption.AllDirectories))
            {
                var filename = Path.GetFileName(file);
                if (IsControlledDocumentFilename(filename))
                {
...
static void InsertAllocation(SqliteConnection connection, SqliteTransaction transaction, string level2, string level3, int number, string freeText, string filename)
{
    using var command = connection.CreateCommand();
    command.Transaction = transaction;
    command.CommandText = @"
INSERT INTO allocations (level1, level2, level3, number, free_text, filename)
VALUES ($level1, $level2, $level3, $number, $freeText, $filename)
ON CONFLICT(level1, level2, level3, number) DO UPDATE SET
    ...
    command.Parameters.AddWithValue("$freeText", freeText);
